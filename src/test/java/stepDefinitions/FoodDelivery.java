...
public class FoodDelivery {
	...
	public void i_send_a_post_request_to_with_the_following_body(String endpoint,
		...
		for (Map<String, String> row : data) {
			response = given().contentType(ContentType.JSON).body(row) // Use each row as the body
					.when().post(endpoint);
		}
	}
	...
	public void i_send_a_post_request_to_with_the_order_details(String url, DataTable dataTable) {
		...
		String requestBody = "{\"userId\":" + rows.get(1).get(0) + ",\"restaurantId\":" + rows.get(1).get(1)
				+ ",\"foodItems\":[\"" + rows.get(1).get(2).replace(", ", "\", \"") + "\"],\"totalPrice\":"
				+ rows.get(1).get(3) + "}";
...
//        requestBody.put("totalPrice", rows.get(0).get(3));
	...
	public void i_send_a_get_request_to_with_order_id(String endpoint, int orderId) {
		response = given().when().get(endpoint + "/" + orderId);
	}
	...
	public void i_send_a_put_request_to_with_the_username_and_email(String endpoint, String username, String email) {
		...
		response = given().contentType("application/json").body(requestBody).when().put(endpoint);
	}
