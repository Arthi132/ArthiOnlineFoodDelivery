Feature: Online Food Delivery Management System API Testing
  ...
  Scenario: Get an order by ID
    ...
    Then the response status should be 200
    And the response should contain the order with userId 5, restaurantId 5, totalPrice 60
   ...
   Scenario: Get restaurant details by ID
    ...
    When I send a PUT request to "/users/15" with the username "newUsername" and email "newEmail@example.com"
    Then the response status should be 200
